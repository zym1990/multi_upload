		...
		growl: function(str){
			...
			**/
			singleGrowl.parent('li').animate({height: (singleGrowl.height() +15)+'px'},str.speed,function(){
				...
				**/
				if(str.effect == 'fade'){
					singleGrowl.css({right: 0, left:0}).delay(dly).fadeIn(str.speed,function(){
						autoHide();
					});
				...
				/**
				* Auto hide function, this will run after the effects are run(above).
				**/
				function autoHide(){
					if(str.sticky != true){
						setTimeout(function(){
							singleGrowl.fadeOut(str.speed, function(){
